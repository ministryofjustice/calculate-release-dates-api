...
read -r user secret < <(echo $(kubectl -n calculate-release-dates-dev get secret calculate-release-dates-client-creds -o json | jq '.data[] |= @base64d' | jq -r '.data.CLIENT_CREDS_CLIENT_ID, .data.CLIENT_CREDS_CLIENT_SECRET'))
