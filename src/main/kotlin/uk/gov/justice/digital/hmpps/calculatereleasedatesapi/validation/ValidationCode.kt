...
enum class ValidationCode(val message: String, val validationType: ValidationType = VALIDATION) {
  ...
  MULTIPLE_SENTENCES_CONSECUTIVE_TO("There are multiple sentences that are consecutive to court case %s count %s. A sentence should only have one other sentence consecutive to it."),
  ...
  OFFENCE_MISSING_DATE("The calculation must include an offence date for court case %s count %s."),
