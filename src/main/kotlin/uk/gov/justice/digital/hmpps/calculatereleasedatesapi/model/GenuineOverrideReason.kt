...
enum class GenuineOverrideReason(val description: String, val displayOrder: Int, val requiresFurtherDetail: Boolean) {
  ...
  ENTER_APPROVED_DATES("Entering approved dates (such as APD, HDCAD and ROTL)", 3, false),
  RELEASE_DATE_ON_WEEKEND_OR_HOLIDAY("The release dates fall on a weekend or public holiday", 4, false),
  CROSS_BORDER_SECTION_RELEASE_DATE("The dates need to be replaced with cross border section release dates", 5, false),
  AGGRAVATING_FACTOR_OFFENCE("One or more offences have been characterised by an aggravating factor (such as terror)", 6, false),
  OTHER("The reason is not on the list", 6, true),
  ;
