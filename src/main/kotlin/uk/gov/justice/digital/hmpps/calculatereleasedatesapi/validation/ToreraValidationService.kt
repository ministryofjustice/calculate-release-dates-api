...
class ToreraValidationService(
  ...
  private fun getToreraSdsOffenceCodes(sentenceAndOffences: List<SentenceAndOffence>) =
    sentenceAndOffences
      .filter { SentenceCalculationType.isToreraEligible(it.sentenceCalculationType, eligibilityType = SentenceCalculationType.Companion.ToreraEligibilityType.SDS) && it.sentenceDate > SDS_DYO_TORERA_START_DATE }
  ...
  private fun getToreraSopcOffenceCodes(sentencesAndOffence: List<SentenceAndOffence>) =
    sentencesAndOffence
      .filter { SentenceCalculationType.isToreraEligible(it.sentenceCalculationType, eligibilityType = SentenceCalculationType.Companion.ToreraEligibilityType.SOPC) && it.sentenceDate < SOPC_TORERA_END_DATE }
      .map { it.offence.offenceCode }
      .toSet()
}
