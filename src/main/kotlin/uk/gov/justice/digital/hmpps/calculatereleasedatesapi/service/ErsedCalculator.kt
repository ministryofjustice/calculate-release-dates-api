...
class ErsedCalculator(val ersedConfiguration: ErsedConfiguration) {
  ...
  fun doesHdcedDateApply(sentence: CalculableSentence, offender: Offender, isMadeUpOfOnlyDtos: Boolean): Boolean {
    return sentence.durationIsGreaterThanOrEqualTo(ersedConfiguration.envelopeMinimum, ersedConfiguration.envelopeMinimumUnit) &&
      sentence.durationIsLessThan(ersedConfiguration.envelopeMaximum, ersedConfiguration.envelopeMaximumUnit) && !offender.isActiveSexOffender && !isMadeUpOfOnlyDtos
  }
  ...
  fun calculateHdced(sentence: CalculableSentence, sentenceCalculation: SentenceCalculation) {
    ...
    if (sentenceCalculation.adjustedDeterminateReleaseDate.isBefore(sentence.sentencedAt.plusDays(ersedConfiguration.minimumCustodialPeriodDays))) {
    ...
    if (sentence.durationIsLessThan(ersedConfiguration.envelopeMidPoint, ersedConfiguration.envelopeMidPointUnit)) {
  ...
  private fun calculateHdcedUnderMidpoint(
    ...
    if (sentence.sentencedAt.plusDays(ersedConfiguration.minimumCustodialPeriodDays)
  ...
  private fun calculateHdcedOverMidpoint(
    ...
    sentenceCalculation.numberOfDaysToHomeDetentionCurfewEligibilityDate =
      sentenceCalculation.numberOfDaysToDeterminateReleaseDate
        .minus(ersedConfiguration.deductionDays + 1) // Extra plus one because we use the numberOfDaysToDeterminateReleaseDate param and not the sentencedAt param
        .plus(adjustedDays)
    ...
    if (sentence.sentencedAt.plusDays(ersedConfiguration.minimumCustodialPeriodDays)
      ...
      sentenceCalculation.breakdownByReleaseDateType[ReleaseDateType.HDCED] =
        ReleaseDateCalculationBreakdown(
          ...
          rulesWithExtraAdjustments = mapOf(CalculationRule.HDCED_GE_MIDPOINT_LT_MAX_PERIOD to AdjustmentDuration(-ersedConfiguration.deductionDays.toInt())),
          adjustedDays = adjustedDays,
          releaseDate = sentenceCalculation.homeDetentionCurfewEligibilityDate!!,
          unadjustedDate = sentenceCalculation.homeDetentionCurfewEligibilityDate!!.plusDays(
            ersedConfiguration.deductionDays,
          ),
        )
    }
  }
  ...
  private fun calculateHdcedMinimumCustodialPeriod(
    ...
    sentenceCalculation.homeDetentionCurfewEligibilityDate = sentence.sentencedAt.plusDays(ersedConfiguration.minimumCustodialPeriodDays)

    sentenceCalculation.breakdownByReleaseDateType[ReleaseDateType.HDCED] =
      ReleaseDateCalculationBreakdown(
        rules = setOf(CalculationRule.HDCED_MINIMUM_CUSTODIAL_PERIOD, parentRule),
        rulesWithExtraAdjustments = mapOf(CalculationRule.HDCED_MINIMUM_CUSTODIAL_PERIOD to AdjustmentDuration(ersedConfiguration.minimumCustodialPeriodDays.toInt())),
        adjustedDays = 0,
        releaseDate = sentenceCalculation.homeDetentionCurfewEligibilityDate!!,
        unadjustedDate = sentence.sentencedAt,
      )
