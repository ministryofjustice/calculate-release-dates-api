...
internal class SentenceTest {
  ...
  fun testSentence() {
    ...
    val sentence = Sentence(offence, duration, sentencedAt, 0, 0, UUID.fromString(uuidString))
    ...
    assertEquals(
      "Sentence(offence=Offence(startedAt=2020-01-01, endedAt=Optional.empty, isScheduleFifteen=false), " +
        "duration=1 days, sentencedAt=2020-01-01, remandInDays=0, taggedBailInDays=0, " +
  ...
  fun testSentenceConcurrentSentences() {
    ...
    val sentence = Sentence(
      ...
      UUID.randomUUID(), mutableListOf(UUID.fromString("219db65e-d7b7-4c70-9239-98babff7bcd5"))
    )
