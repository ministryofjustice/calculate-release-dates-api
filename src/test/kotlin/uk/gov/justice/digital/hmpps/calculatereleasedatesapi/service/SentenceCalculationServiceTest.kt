...
class SentenceCalculationServiceTest {
  ...
  fun `Example 10`() {
    ...
    adjustments[AdjustmentType.REMAND] = listOf(Adjustment(appliesToSentencesFrom = sentence.sentencedAt, numberOfDays = 35))
    adjustments[AdjustmentType.TAGGED_BAIL] = listOf(Adjustment(appliesToSentencesFrom = sentence.sentencedAt, numberOfDays = 10))
  ...
  fun `Example 11`() {
    ...
    sentenceIdentificationService.identify(sentence, offender)
    val adjustments = mutableMapOf<AdjustmentType, List<Adjustment>>()
    adjustments[AdjustmentType.REMAND] = listOf(Adjustment(appliesToSentencesFrom = sentence.sentencedAt, numberOfDays = 10))
    val offender = Offender("A1234BC", "John Doe", LocalDate.of(1980, 1, 1))
    val booking = Booking(offender, mutableListOf(sentence), adjustments)
    val calculation = sentenceCalculationService.calculate(sentence, booking)
  ...
  fun `Example 12`() {
    ...
    sentenceIdentificationService.identify(sentence, offender)
    val adjustments = mutableMapOf<AdjustmentType, List<Adjustment>>()
    adjustments[AdjustmentType.REMAND] = listOf(Adjustment(appliesToSentencesFrom = sentence.sentencedAt, numberOfDays = 21))
    val offender = Offender("A1234BC", "John Doe", LocalDate.of(1980, 1, 1))
    val booking = Booking(offender, mutableListOf(sentence), adjustments)
    val calculation = sentenceCalculationService.calculate(sentence, booking)
