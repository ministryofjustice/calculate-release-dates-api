...
class FTR56TrancheSelectionStrategyTest {
  ...
  inner class SentenceDurationsWithinTrancheDuration {
    ...
    fun `should return true if all durations are less than the tranche duration`() {
      ...
      assertThat(FTR56TrancheSelectionStrategy.sentenceDurationsWithinTrancheDuration(trancheConfig, durations)).isTrue
    }

    @Test
    fun `should return true if any duration is equal to the tranche duration`() {
      val trancheConfig = TrancheConfiguration(
        type = TrancheType.SENTENCE_LENGTH,
        date = LocalDate.now(),
        duration = 4 * 365,
    ...
    fun `should return false if any duration is greater than the tranche duration`() {
      ...
      assertThat(FTR56TrancheSelectionStrategy.sentenceDurationsWithinTrancheDuration(trancheConfig, durations)).isFalse
    }
