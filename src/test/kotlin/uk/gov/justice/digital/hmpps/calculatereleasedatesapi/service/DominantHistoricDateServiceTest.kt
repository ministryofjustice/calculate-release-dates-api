...
class DominantHistoricDateServiceTest {
  ...
  fun `should use historic SLED if it exists and no newer LED or SED`() {
    val historicSled = LocalDate.of(2020, 1, 1)
    val outcomes = listOf(
      CalculationOutcome(
    ...
    val overrides = service.calculateFromSled(LocalDate.of(2021, 1, 1), outcomes)
  ...
  fun `should calculate LED and SED when historic LED is after historic SLED`() {
    val sled = LocalDate.of(2020, 1, 1)
    ...
    val overrides = service.calculateFromSled(sled, outcomes)

    assertEquals(sled, overrides[ReleaseDateType.SED]) // no historic SED
  ...
  @Test
  fun `should fall back to passed-in SLED when no historic SLED`() {
    val sled = LocalDate.of(2022, 1, 1)
    val outcomes = listOf(
      CalculationOutcome(
        calculationRequestId = 1L,
    ...
    assertEquals(sled, overrides[ReleaseDateType.LED])
    assertFalse(overrides.containsKey(ReleaseDateType.SLED))
  }
